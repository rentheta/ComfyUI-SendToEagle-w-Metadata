...
この拡張機能は、Eagleへのアイテム送信を同期的に行う代わりに、バックグラウンドスレッドで行うことができます。
これにより画像生成中にEagleを待つ時間を短縮できる可能性がありますが、代わりにメモリ内キューを使用します。

### 有効化方法

ComfyUIを起動する**前に**、環境変数 `EAGLE_ASYNC_SEND` を `1` に設定してください。
...
非同期送信を無効にするには、`EAGLE_ASYNC_SEND` を設定しないか、設定を解除してターミナルを閉じ、
ComfyUIを再起動してください。

### 動作と制限事項

- `EAGLE_ASYNC_SEND=1` が設定されている場合、Eagleへの送信は別スレッドで実行されます。
- ComfyUIでの画像処理が完了した後も、Eagleへの送信はバックグラウンドで継続されます。
- 送信キューは**メモリ内のみ**です。Python/ComfyUIが強制終了された場合
  （クラッシュ、`kill -9`、電源喪失など）、キューに残っているアイテムは**失われる**可能性があり、
  Eagleに届かないことがあります。
- この機能は**実験的**であり、今後のバージョンで動作や利用可能性が変更される可能性があります。
- Eagleへの送信に問題があると疑われる場合は、`EAGLE_ASYNC_SEND` を**設定せず**に再度実行して、
  同期送信に戻してください。
  
## 変更履歴
- 2025/12/17 1.2.0 環境変数(`EAGLE_ASYNC_SEND`)が設定されていた場合、Eagleへの送信を別スレッドで行う（実験的オプション）
